...
int get_aptime( string fname, int ser ) {
    ifstream iss(fname);
    string buffer;
    istringstream line;

    if (!iss) {
        cerr << "ERROR: " << fname << " not found." << endl;
        exit(-1);
    }
    ...
    while (getline(iss, buffer)) {
        ...
        if (line >> id >> date >> time) {
            if (id % 1000 == ser) return(time);
        }
    }

    //cerr << "ERROR: Could not match serial number" << endl;
    return(-999);
    //exit(-1);
    //return(0);
}
...
vector<Node*> initialize_1node( int aptime, double intensity, int pop_multiplier ) {
    vector<Node*> nodes;
    ...
    if (aptime + 2 > 0) {
        TimeSeriesAnchorPoint a = {217, intensity};
        TimeSeriesAnchorPoint b = {217 + aptime + 2, 0.11};
        TimeSeriesAnchorPoint c = {400, 0.11};
        Ki_ap.push_back(a);
        Ki_ap.push_back(b);
        Ki_ap.push_back(c);
    } else if (aptime == -999) {
        TimeSeriesAnchorPoint a = {217, intensity};
        TimeSeriesAnchorPoint c = {400, intensity};
        Ki_ap.push_back(a);
        Ki_ap.push_back(c);
    } else {
        TimeSeriesAnchorPoint c = {400, 0.11};
...
void runsim (int serial, string fname, double intensity, int pop_multiplier) {
    cout << "Running Sim " << to_string(serial) << endl;

    vector<string> out_buffer;
    vector<string> out_buffer2;
    vector<Node*> nodes = initialize_1node(get_aptime(fname, serial % 1000), intensity, pop_multiplier);
...
int main(int argc, char* argv[]) { 
    ...
    int pop_multiplier = std::stoi(argv[4]);

    runsim(serial, fname, intensity, pop_multiplier);
