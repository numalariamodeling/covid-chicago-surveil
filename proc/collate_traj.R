...
bigdf <- data.frame()
for (i in ini:fin) {
  df <- data.table::fread(paste0("/projects/b1139/covid-age-output/daily_output.", i))
  df$serial <- i
...
bigdf1 <- bigdf %>% group_by(serial, node) %>%
  ...
  mutate(norm_new_sym = new_sym / max(new_sym, na.rm = T),
         norm_new_adm = new_adm / max(new_adm, na.rm = T))
...
bigdf1 %>% group_by(node, time) %>% 
  summarise(Ki = mean(Ki)) %>% 
  ggplot() + 
  geom_line(aes(x=time, y=Ki, colour=as.factor(node)))

ggplot(bigdf2) +
  geom_line(aes(x=date, y=norm_new_sym, group=serial), colour="black", alpha=0.2) +
  geom_line(aes(x=date, y=norm_new_adm, group=serial), colour="blue", alpha=0.2)
  # geom_line(aes(x=date, y=norm_hosp, group=serial), colour="red", alpha=0.2)
